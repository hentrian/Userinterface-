...
begin
  if(edtpasswort.Text='')or (edtpasswortwdh.text= '') or (edtvorname.Text='')or (edtnachname.Text='')or (edtbenutzername.Text='')then
    showMessage('Es ist essentiell Benutzername, Passwort und Ihren Vor- und Nachnamen einzugeben! Bitte überprüfen sie ihre Eingabe!')
  else
            ...
            begin
              if not fileexists(edtbenutzername.Text + '.txt') then
                begin
                  ...
                  registieren:= true;
                  showMessage('Gratuliere. Sie sind nun registriert und können sich jetzt einloggen!');
                end
              else
                showMessage('Jemand mit dem gleichen Benutzernamen existiert bereits! Bitte nutzen Sie einen anderen Benutzernamen!')
            end
          else
            showMessage('Ihre Passwörter stimmen nicht überein');
         end
end;
